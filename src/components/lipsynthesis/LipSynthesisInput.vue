<template>
  <div class="container">
    <p style="color: #fff; font-size: 1.2em; margin: 0 auto;">请输入或粘贴一段文字进行唇形合成</p>
  ...
  methods: {
    onNext() {
      if (this.text.length < 10 || this.text.length > 50)
        return
      this.$router.push({path: 'lipsynthesis', query: Object.assign({}, this.$route.query, {text: this.text})})
    }
  },
  computed: {
    count() {
